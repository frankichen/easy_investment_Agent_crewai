...
**推荐方式：使用UI界面配置（无需手动编辑文件）**

1. 启动UI程序后，点击右上角的"⚙️ AI配置"按钮
2. 在配置页面填写您的AI参数：
   - API Key（必填）
   - Base URL（默认：https://api.openai.com/v1）
   - Model Name（默认：gpt-4o）
   - Temperature（默认：0.8）
   - Max Tokens（默认：14000）
3. 点击"💾 保存设置"
4. 配置会自动保存到 `~/.a_stock_analysis/ai_settings.json`
...
**注意**：UI配置的优先级高于环境变量。如果在UI中设置了配置，将使用UI的配置。
...
1. **打开配置页面**
   - 点击主页面右上角的"⚙️ AI配置"按钮
   - 或直接访问 http://localhost:8080/settings

2. **填写配置信息**
   - **API Key**（必填）：您的OpenAI API密钥
   - **Base URL**：API服务地址，如使用代理或第三方服务可修改
   - **Model Name**：选择使用的AI模型（如 gpt-4o、gpt-4、gpt-3.5-turbo）
   - **Temperature**：调整输出的创造性（0.0 = 保守，1.0 = 创新）
   - **Max Tokens**：控制单次生成的长度
...
**配置存储位置**：`~/.a_stock_analysis/ai_settings.json`
